...
print_node_info()
{
  LOG_INFO("HCK-NODE root %u %x %u\n", iotlab_nodes[0][0], iotlab_nodes[0][1], IOTLAB_ROOT_ID);
  uint8_t i = 1;
  for(i = 1; i < NODE_NUM; i++) {
    LOG_INFO("HCK-NODE non_root %u %x %u\n", iotlab_nodes[i][0], iotlab_nodes[i][1], i + 1);
  }
...
#elif TESTBED_SITE == IOTLAB_LILLE_32
uint16_t root_info[3] =  {89, 0xa990, 0};
uint16_t non_root_info[NON_ROOT_NUM][3] = {
  {46, 0xb772, 0},
  {47, 0x1854, 0},
  {48, 0x956, 0},
