...
reset_eval(uint8_t phase)
{
  ...
  LOG_HCK("eval_config 3 with_top s_b offs %u thr %u fco %u nco %u que %u esc %u sel %u retx %u bac %u |\n", 
          QUICK_NUM_OF_OFFSETS,
          QUICK_OFFSET_ASSIGNMENT_BC_PKTS_CRITICAL_THRESH,
          QUICK_OFFSET_ASSIGNMENT_CRITICAL_PKTS_OFFSET,
          QUICK_OFFSET_ASSIGNMENT_NON_CRITICAL_PKTS_OFFSET,
          QUICK_DUPLICATE_PACKET_MANAGEMENT,
          QUICK_OFFSET_ESCALATION,
          QUICK_OFFSET_BASED_PACKET_SELECTION,
          QUICK_NO_TX_COUNT_INCREASE_FOR_POSTPONED_PACKETS,
          QUICK_BACKOFF_FOR_BCAST_PACKETS);
#endif
#endif
#endif
}
         ...
         uint16_t datalen)
{
  ...
  if(sender_index == NODE_NUM) {
    LOG_HCK("| Fail to receive: out of index\n");
    return;
  }
  ...
  if(app_up_sequence_is_duplicate(sender_id, app_received_seqno_count)) {
    LOG_HCK("| dup_up from %u a_seq %lx at %llx\n", 
              sender_id,
              app_received_seqno,
              app_rx_up_asn);
  } else {
    ...
    LOG_HCK("rx_up %u | from %u a_seq %lx len %u lt_up_r %llx lt_up_t %llx hops %u\n", 
              iotlab_nodes[sender_index][2], //
              sender_id,
              app_received_seqno,
              datalen,
  ...
  while(1) {
    ...
    if(data == &print_node_info_timer) {
      ...
      if(data_count <= APP_DOWNWARD_MAX_TX) {
        ...
        LOG_HCK("tx_down %u | to %u a_seq %lx len %u at %llx\n", 
                  data_count,
                  dest_id,
                  app_seqno,
                  current_payload_len, 
